-- Migration: 20260413000002_award_drops_inline_leaderboard_score_update.sql
-- Description: Fix "No rankings yet" after workout — update leaderboard_live_scores
--              inline in award_drops so the user appears on the leaderboard immediately
--              instead of waiting up to 5 minutes for the cron refresh.
--
-- AGENT NOTE: [2026-04-13] - supabase-dba
--
-- ROOT CAUSE:
--   award_drops writes drops to sessions/profiles/gym_memberships synchronously.
--   leaderboard_live_scores is only refreshed by a cron every 5 minutes.
--   get_leaderboard('gym') filters WHERE score_val > 0, so a user whose row in
--   leaderboard_live_scores doesn't exist yet (or still has 0) is invisible.
--   Result: "No rankings yet" immediately after finishing a workout.
--
-- FIX:
--   Append a single UPSERT into leaderboard_live_scores at the end of award_drops'
--   core write block. Only adds the session's drops delta — does NOT re-aggregate
--   all sessions (that's the cron's job). This ensures the user appears immediately
--   with at least their current session's contribution.
--
-- CHANGES:
--   - Modified function: public.award_drops
--     Added inline upsert of leaderboard_live_scores after the gym_memberships update,
--     incrementing weekly_score + monthly_score + alltime_score by v_final_drops.
--
-- IMPACT ON FRONTEND:
--   - Mobile App: User appears on leaderboard immediately after workout ends.
--     Score may still be slightly off until next 5-min cron (accurate after that).
--   - Admin Panel: No change
--
-- BREAKING CHANGES: None
--
-- NOTE ON CORRECTNESS:
--   The inline update adds v_final_drops to the existing scores. The 5-min cron
--   performs a full re-aggregate (SUM from sessions) which naturally supercedes
--   the inline increment — so there is no double-counting risk. The cron uses
--   DO UPDATE SET weekly_score = EXCLUDED.weekly_score (full replace), which
--   overwrites the incremented value with the correct aggregate.
...
BEGIN
  ...
  IF v_final_drops > 0 THEN
    ...
    -- ── Inline leaderboard score update ─────────────────────────
    -- Ensures the user appears on the leaderboard immediately after their workout
    -- rather than waiting up to 5 minutes for the refresh_leaderboard_live_scores cron.
    -- The 5-min cron will overwrite this with the fully-accurate aggregate.
    -- We use the week/month timestamps already computed above for consistency.
