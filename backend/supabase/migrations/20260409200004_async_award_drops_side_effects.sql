-- Migration: 20260409200004_async_award_drops_side_effects.sql
-- Description: Decouple award_drops side effects into an async queue.
--              award_drops now does ONLY the core write (session + profile + wallet +
--              transaction) and enqueues side effects. A cron job processes the queue.
--
-- AGENT NOTE: [2026-04-09] - supabase-dba
--
-- Before: award_drops holds FOR UPDATE lock on sessions row while executing:
--   calculate_session_drops_v2, get_active_drop_boost, update_challenge_progress,
--   update_arena_scores, evaluate_badges, refresh_economy_snapshot_daily,
--   update_checkin_challenge_progress
-- Total: ~486 lines, 8 tables, 8 function calls, all in one transaction.
-- At 50 concurrent finishes, lock contention causes timeouts.
--
-- After: award_drops does core write (~200ms) and INSERTs a queue row.
-- process_pending_side_effects() cron runs every minute to handle the rest.
-- The mobile app gets drops_earned back instantly; badges arrive via Realtime.
--
-- CHANGES:
--   - Created pending_session_side_effects queue table
--   - Created process_pending_side_effects() function
--   - Scheduled cron job every 1 minute
--   - Rewrote award_drops to enqueue instead of inline side effects
--
-- IMPACT ON FRONTEND:
--   - Mobile App: award_drops returns faster. Badges/challenge progress may
--     appear up to 60s later (delivered via Realtime user_badges subscription).
--   - Admin Panel: No change
--
-- BREAKING CHANGES: None (award_drops return type unchanged)

-- ============================================================
-- 1. Queue table
-- ============================================================
CREATE TABLE IF NOT EXISTS public.pending_session_side_effects (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id  UUID NOT NULL,
  user_id     UUID NOT NULL,
  gym_id      UUID NOT NULL,
  drops_earned INT NOT NULL DEFAULT 0,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  processed_at TIMESTAMPTZ,
  error_message TEXT
);

CREATE INDEX IF NOT EXISTS idx_pending_side_effects_unprocessed
  ON public.pending_session_side_effects (created_at ASC)
  WHERE processed_at IS NULL;

-- No RLS needed: only accessed by SECURITY DEFINER functions and cron
ALTER TABLE public.pending_session_side_effects ENABLE ROW LEVEL SECURITY;

-- ============================================================
-- 2. Process queue function
-- ============================================================
CREATE OR REPLACE FUNCTION public.process_pending_side_effects()
RETURNS INTEGER
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      RAISE WARNING 'Side effect processing failed for session %: %', v_item.session_id, SQLERRM;
    END;
  END LOOP;

  -- Cleanup: remove processed items older than 24h
  DELETE FROM public.pending_session_side_effects
...
-- ============================================================
-- 3. Schedule cron: every 1 minute
-- ============================================================
DO $outer$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    PERFORM cron.unschedule('process-session-side-effects');
  END IF;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    PERFORM cron.schedule(
      'process-session-side-effects',
      '* * * * *',
      $$SELECT public.process_pending_side_effects();$$
    );
    RAISE NOTICE 'pg_cron: process-session-side-effects scheduled every 1 minute.';
  END IF;
END $outer$;

-- ============================================================
-- 4. Rewrite award_drops: core write only, enqueue side effects
-- ============================================================
...
BEGIN
  ...
  -- (identical logic to original, abbreviated variable names)
  -- ═══════════════════════════════════════════════════════════════
  ...
  -- This is the only part that MUST be synchronous.
  -- ═══════════════════════════════════════════════════════════════
  ...
  -- Challenge progress, arena scores, badges, economy snapshot,
  -- implicit checkin are processed by process_pending_side_effects()
  -- ═══════════════════════════════════════════════════════════════
