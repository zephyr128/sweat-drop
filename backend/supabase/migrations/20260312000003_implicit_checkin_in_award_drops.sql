-- ═══════════════════════════════════════════════════════════
-- Migration: 20260312000003_implicit_checkin_in_award_drops.sql
-- Description: Add implicit check-in to award_drops() — cardio sessions count as gym visits
--
-- AGENT NOTE: [2026-03-12] - supabase-dba
-- Reference: docs/plans/master_execution_plan.md — Phase 1, Migration C
--
-- CHANGES:
--   - award_drops() now inserts an implicit gym_checkin (drops_earned=0, gps_verified=false)
--   - Also calls update_checkin_challenge_progress() for check-in challenges
--
-- IMPACT ON FRONTEND:
--   - Mobile: Users who do a workout session also get credit for a check-in visit
--   - Admin: Check-in stats will include implicit check-ins (gps_verified=false, drops=0)
--
...
BEGIN
  ...
  -- 15. IMPLICIT CHECK-IN — cardio session counts as gym visit
  -- drops_earned=0 (drops already awarded via session), gps_verified=false (no QR scan)
...
COMMENT ON FUNCTION public.award_drops(UUID) IS
  ...
  'Also inserts implicit gym check-in (drops=0, gps=false) and updates check-in challenge progress.';
