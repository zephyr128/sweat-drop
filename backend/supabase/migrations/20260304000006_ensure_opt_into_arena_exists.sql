-- Migration: 20260304000006_ensure_opt_into_arena_exists.sql
-- Description: Ensures opt_into_arena() RPC function exists and is properly granted
-- 
-- AGENT NOTE: [2026-03-04] - supabase-dba
-- Problem: opt_into_arena() function not found in schema cache
-- 
-- CHANGES:
-- - Recreate opt_into_arena() RPC function
-- - Ensure proper grants are set
-- - Refresh schema cache by recreating function
...
-- PostgREST caches function signatures, so we need to notify it of changes
-- This is done by ensuring the function exists and has proper grants
...
BEGIN
  ...
  RAISE NOTICE 'opt_into_arena() function verified and ready';
END $$;
