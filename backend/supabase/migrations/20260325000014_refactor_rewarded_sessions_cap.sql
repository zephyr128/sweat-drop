-- Migration: 20260325000014_refactor_rewarded_sessions_cap.sql
-- Description: Refactor rewarded-sessions-per-day from hard block to configurable
--   soft/hard/off policy. Add BLE restart-fragment reconciliation.
--
-- CHANGES:
--   - Added columns on tokenomics_config:
--       enforce_rewarded_sessions_cap BOOLEAN DEFAULT false
--       rewarded_sessions_cap_mode TEXT DEFAULT 'soft' ('off','soft','hard')
--       session_restart_grace_sec INTEGER DEFAULT 300
--   - Updated award_drops() to respect cap mode + restart grace logic
--   - Updated get_user_drop_limits() to return new policy fields
--
-- IMPACT ON FRONTEND:
--   - Mobile App: get_user_drop_limits now returns rewarded_sessions_cap_mode and
--     session_restart_grace_sec. Mobile should show soft-cap warnings, not blocks.
--   - Admin Panel: new fields on tokenomics_config for economy settings.
--
-- BREAKING CHANGES: None (defaults preserve existing behavior as soft signal).

-- ============================================================================
-- 1. Schema: add columns to tokenomics_config
-- ============================================================================

ALTER TABLE public.tokenomics_config
  ADD COLUMN IF NOT EXISTS enforce_rewarded_sessions_cap BOOLEAN NOT NULL DEFAULT false;

ALTER TABLE public.tokenomics_config
  ADD COLUMN IF NOT EXISTS rewarded_sessions_cap_mode TEXT NOT NULL DEFAULT 'soft';
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND constraint_name = 'chk_rewarded_sessions_cap_mode'
  ) THEN
    ALTER TABLE public.tokenomics_config
      ADD CONSTRAINT chk_rewarded_sessions_cap_mode
      CHECK (rewarded_sessions_cap_mode IN ('off','soft','hard'));
  END IF;
END $$;

ALTER TABLE public.tokenomics_config
  ADD COLUMN IF NOT EXISTS session_restart_grace_sec INTEGER NOT NULL DEFAULT 300;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.tokenomics_config
      ...
      CHECK (session_restart_grace_sec BETWEEN 0 AND 1800);
  END IF;
END $$;

-- Normalize: ensure enforce flag is false everywhere initially (soft rollout)
UPDATE public.tokenomics_config
SET enforce_rewarded_sessions_cap = false,
    rewarded_sessions_cap_mode = 'soft'
WHERE enforce_rewarded_sessions_cap IS DISTINCT FROM false
   OR rewarded_sessions_cap_mode IS DISTINCT FROM 'soft';

-- Ensure max_rewarded_sessions_per_day >= 1
UPDATE public.tokenomics_config
SET max_rewarded_sessions_per_day = 4
WHERE max_rewarded_sessions_per_day < 1;

-- ============================================================================
-- 2. award_drops() — refactored rewarded-sessions cap + restart reconciliation
-- ============================================================================
...
BEGIN
  ...
  -- ── Fetch tokenomics config including new policy fields ──
  SELECT
  ...
  -- Count how many of today's rewarded sessions are restart fragments
  -- (same user + same machine, started within grace window of a previous session ending)
  ...
  IF v_cap_mode = 'off' THEN
    -- No cap enforcement at all; proceed to session/day/week caps
    NULL;
  ...
  ELSIF v_cap_mode = 'hard' AND v_effective_rewarded_sessions >= v_max_sessions_day THEN
    PERFORM public.log_fraud_event(v_session.user_id, v_session.gym_id,
      ...
      jsonb_build_object(
        ...
        'actual_rewarded', v_rewarded_sessions_today,
  ...
  ELSIF v_cap_mode = 'soft' AND v_effective_rewarded_sessions >= v_max_sessions_day THEN
    ...
    -- Soft mode: do NOT block, proceed to session/day/week caps
  END IF;

  -- If hard-blocked above, v_final_drops is already 0; skip remaining caps
  ...
  -- ── Persist restart merge info and cap reason in drop_calc_v2 metadata ──
  IF v_restart_merged_count > 0 THEN
    ...
    IF v_drop_calc_v2->'reasons' IS NOT NULL THEN
      v_drop_calc_v2 := jsonb_set(v_drop_calc_v2, '{reasons}',
        (v_drop_calc_v2->'reasons') || to_jsonb('session_restart_merged'::TEXT));
    END IF;
  END IF;
  ...
  IF v_cap_reason IS NOT NULL THEN
    ...
    IF v_drop_calc_v2->'reasons' IS NOT NULL THEN
      v_drop_calc_v2 := jsonb_set(v_drop_calc_v2, '{reasons}',
        (v_drop_calc_v2->'reasons') || to_jsonb(v_cap_reason));
    END IF;
  END IF;
...
-- ============================================================================
-- 3. get_user_drop_limits() — return new policy fields
