-- Migration: 20260326000002_dashboard_v3_activity_workouts.sql
-- Description: Dashboard V3 with workout events in desk feed + Activity Log RPC with workout support
--
-- AGENT NOTE: [2026-03-26] - supabase-dba
-- Reference: docs/plans/admin_dashboard_premium_v3_plan.md
--            docs/plans/admin_dashboard_v3_workout_activity_addendum.md
--
-- Supersedes: 20260326000001_dashboard_v3_rpc.sql (adds workout events)
--
-- WORKOUT EVENT CLASSIFICATION:
--   workout_started       → sessions WHERE is_active = true (or started_at for historical)
--   workout_finished      → sessions WHERE is_active = false AND no auto_cancel_reason
--   workout_auto_finished → sessions WHERE is_active = false AND raw_metrics->'security'->>'auto_cancel_reason' IS NOT NULL
--   workout_cancelled     → not reliably detectable yet (no end_reason column)
--
-- TODO: Follow-up migration to add sessions.end_reason TEXT column for normalized classification.
--       Until then, classification uses raw_metrics->'security'->>'auto_cancel_reason' heuristic.
--
-- IMPACT ON FRONTEND:
--   Admin Panel:
--     - deskFeed items may now have kind = 'workout_finished' | 'workout_auto_finished'
--     - Activity Log supports p_kind = 'workout' filter
--     - Activity Log items may have kind = 'workout_started' | 'workout_finished' | 'workout_auto_finished'
...
CREATE INDEX IF NOT EXISTS idx_sessions_gym_started_at
  ON public.sessions (gym_id, started_at DESC);

CREATE INDEX IF NOT EXISTS idx_sessions_gym_active_updated
  ON public.sessions (gym_id, is_active, updated_at DESC);
...
-- ============================================================
-- 1) REWRITE get_gym_dashboard_overview — all metric fixes + workout feed
-- ============================================================
...
BEGIN
  ...
  -- ── Desk Feed (last 10 mixed: checkins + redemptions + workout_finished + workout_auto_finished) ──
  -- NOTE: workout_started deliberately excluded (noise control per addendum)
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
  ...
  FROM (
    ...
    (SELECT s.id::TEXT,
            ...
            COALESCE(p.username, 'Member')
              || CASE
                   WHEN s.raw_metrics->'security' ? 'auto_cancel_reason'
                   THEN ' auto-finished on '
  ...
  -- ── Top Performers (earned drops, role='user', top 5) ──
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
...
-- ============================================================
-- 2) Activity Log RPC — with workout events
-- ============================================================
...
DECLARE
  ...
  v_items  JSONB;
  v_search TEXT;
  v_include_checkins    BOOLEAN;
  v_include_redemptions BOOLEAN;
  v_include_workouts    BOOLEAN;
BEGIN
  IF NOT public._admin_check_gym_access(p_gym_id) THEN
    RETURN jsonb_build_object('error', 'Unauthorized');
  END IF;
  ...
  v_include_checkins    := (p_kind = 'all' OR p_kind = 'checkin');
  v_include_redemptions := (p_kind = 'all' OR p_kind = 'redemption');
  v_include_workouts    := (p_kind = 'all' OR p_kind = 'workout');
  ...
  WITH activity AS (
    ...
    -- Workout events: started, finished, auto_finished
    -- Each session generates TWO activity items:
    --   1) workout_started (from started_at)
    --   2) workout_finished OR workout_auto_finished (from ended_at/updated_at, if not active)
    SELECT
      s.id::TEXT || '_start',
      'workout_started'::TEXT,
      COALESCE(p.username, p.full_name, 'Member'),
      p.avatar_url,
      'Started workout on ' || COALESCE(m.name, m.type, 'machine'),
      'active'::TEXT,
      s.started_at
    FROM public.sessions s
    JOIN public.profiles p ON p.id = s.user_id
    ...
    SELECT
      ...
      CASE
        WHEN s.raw_metrics->'security' ? 'auto_cancel_reason'
        THEN 'Auto-finished on ' || COALESCE(m.name, m.type, 'machine')
        ELSE 'Finished workout on ' || COALESCE(m.name, m.type, 'machine')
             || CASE WHEN s.duration_seconds IS NOT NULL
                  THEN ' (' || (s.duration_seconds / 60) || ' min'
                       || CASE WHEN s.drops_earned > 0 THEN ', +' || s.drops_earned || ' drops' ELSE '' END
                       || ')'
                  ELSE ''
                END
