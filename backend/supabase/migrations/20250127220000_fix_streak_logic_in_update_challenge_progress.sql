-- Fix Streak Logic in update_challenge_progress Function
-- Corrects streak tracking logic to properly handle consecutive days
-- Ensures completed_now is returned correctly when streak reaches target
  ...
  LOOP
    ...
    CASE v_challenge.challenge_type
      ...
      WHEN 'streak' THEN
        ...
        -- Streak logic:
        -- - If last_activity_date is NULL (first time) -> Set current_streak_days = 1
        -- - If last_activity_date == p_session_date -> Don't change (already recorded today)
        -- - If last_activity_date == p_session_date - 1 -> Increment current_streak_days by 1
        -- - If last_activity_date < p_session_date - 1 -> Reset streak to 1 (gap in continuity)
        ...
        DO UPDATE SET
          current_streak_days = CASE
            -- If last_activity_date is NULL (first time) -> Set to 1
            WHEN challenge_progress.last_activity_date IS NULL THEN 1
            -- If same day -> Don't change (already recorded today)
            WHEN challenge_progress.last_activity_date = p_session_date 
              THEN challenge_progress.current_streak_days
            -- If next day (consecutive) -> Increment by 1
        ...
        -- Return completed_now = true when current_streak_days becomes equal to streak_days
