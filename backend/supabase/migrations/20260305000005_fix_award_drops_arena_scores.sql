-- Migration: 20260305000005_fix_award_drops_arena_scores.sql
-- Description: Fix award_drops() to call update_arena_scores() - missing call was preventing arena score updates
-- 
-- AGENT NOTE: [2026-03-05] - supabase-dba
-- 
-- PROBLEM:
-- award_drops() function does NOT call update_arena_scores(), so arena scores never update
-- This was identified via DIAGNOSE_ARENA_SCORE_ISSUE.sql STEP 7 check
-- 
-- ROOT CAUSE:
-- Migration 20260303100004_update_award_drops_for_arenas.sql added the call, but it seems
-- award_drops() was recreated by a later migration that didn't include update_arena_scores()
-- 
-- CHANGES:
-- - Add step 13b in award_drops() to call update_arena_scores() after challenge progress update
-- - This ensures arena scores are updated in real-time when award_drops() is called
-- 
-- IMPACT ON FRONTEND:
-- - Mobile App: Arena scores will now update automatically after workout sessions
-- 
-- BREAKING CHANGES:
-- - None (additive change)

-- Read current award_drops() function and add update_arena_scores() call
...
BEGIN
  ...
  -- FIX: This call was missing, causing arena scores to never update
...
COMMENT ON FUNCTION public.award_drops(UUID) IS
  ...
  'FIXED: Now includes update_arena_scores() call to update arena scores in real-time.';
