-- Migration: 20260330000006_fix_award_drops_column_names.sql
-- Description: Fix two wrong column references in award_drops() tokenomics config load.
--   tc.restart_grace_period_sec → tc.session_restart_grace_sec
--   tc.merge_window_sec → tc.split_merge_window_sec
-- Without this fix, the entire config load silently fails (EXCEPTION WHEN OTHERS THEN NULL)
-- and the function uses hardcoded defaults instead of gym-specific config.
