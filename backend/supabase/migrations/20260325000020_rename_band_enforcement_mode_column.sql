-- Migration: 20260325000020_rename_band_enforcement_mode_column.sql
-- Description: Rename reward_band_enforcement_mode → band_enforcement_mode to match admin panel contract
--
-- AGENT NOTE: [2026-03-25] - supabase-dba
-- Admin panel already references 'band_enforcement_mode'. Column was created as
-- 'reward_band_enforcement_mode' in previous migration. Rename for consistency.

-- 1) Rename the column
ALTER TABLE public.tokenomics_config
  RENAME COLUMN reward_band_enforcement_mode TO band_enforcement_mode;

-- 2) Drop old constraint name and re-add with correct name
ALTER TABLE public.tokenomics_config
  DROP CONSTRAINT IF EXISTS chk_reward_band_enforcement_mode;

ALTER TABLE public.tokenomics_config
  DROP CONSTRAINT IF EXISTS chk_band_enforcement_mode;

ALTER TABLE public.tokenomics_config
  ADD CONSTRAINT chk_band_enforcement_mode
  CHECK (band_enforcement_mode IN ('warn', 'enforce'));

-- 3) Update claim_reward() to reference the renamed column
