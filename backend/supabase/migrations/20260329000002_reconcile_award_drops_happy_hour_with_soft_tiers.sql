...
-- Description: P0 FIX — Reconcile award_drops to include BOTH:
--   (A) Happy Hour boost (from 20260327000005)
--   (B) Session soft tiers + anti-split merge + cap modes (from 20260325000016)
--
-- The 270005 migration inadvertently overwrote the sophisticated award_drops from
-- migration 16, dropping soft tiers, anti-split merge, and cap modes. This caused
-- backend/mobile mismatch: mobile's live-drops-estimator.ts applies soft tiers
-- but the backend was using hard session cap.
--
-- Order of operations:
...
--
-- ROLLBACK: Re-apply 20260327000005 award_drops body to revert to hard-cap-only.
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- TOKENOMICS CONFIG (all columns, gym-specific then global fallback)
  -- ═══════════════════════════════════════════════════════════════
  SELECT
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- HAPPY HOUR BOOST (applied to v_raw_drops BEFORE soft tiers)
  -- ═══════════════════════════════════════════════════════════════
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- SOFT SESSION THRESHOLD + ANTI-SPLIT MERGE + DAILY/WEEKLY CAPS
  -- (only if not hard-blocked by rewarded_sessions_cap)
  -- ═══════════════════════════════════════════════════════════════
  IF v_cap_reason IS DISTINCT FROM 'rewarded_sessions_cap_hard_block' THEN

    -- Anti-split: aggregate drops from recent adjacent sessions within merge window
    ...
    -- Piecewise soft-threshold on boosted v_raw_drops + merged prior
    ...
    -- Marginal credit for this session
