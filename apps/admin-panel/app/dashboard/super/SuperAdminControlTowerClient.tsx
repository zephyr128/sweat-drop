...
interface GymWithOwner {
  ...
  /** Whether the gym appears in the mobile app gym list */
...
export default function SuperAdminControlTowerClient() {
  ...
  const loadData = async () => {
    ...
    if (gymsResult.success && gymsResult.data) {
      const raw = gymsResult.data as GymWithOwner[];
      setGyms(raw.map((g) => ({ ...g, is_mobile_listed: g.is_mobile_listed ?? true })));
    }
          ...
          <table className="w-full">
            <thead className="bg-[#1A1A1A]">
              <tr>
                ...
                <th className="px-6 py-4 text-center text-sm font-medium text-[#808080] uppercase tracking-wide">
                  Mobile
                </th>
            ...
            <tbody className="divide-y divide-[#1A1A1A]">
              {gyms.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-[#808080]">
                    No gyms found
                  </td>
                </tr>
              ) : (
                gyms.map((gym) => (
                  <tr key={gym.gym_id} className="hover:bg-[#1A1A1A]/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <PilotVisibilityToggle
                        gymId={gym.gym_id}
                        initialEnabled={gym.is_mobile_listed}
                        compact
                      />
                    </td>
                    <td className="px-6 py-4">
