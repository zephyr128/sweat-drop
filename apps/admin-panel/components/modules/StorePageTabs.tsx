...
export function StorePageTabs({ gymId }: StorePageTabsProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const activeTab = (searchParams.get('tab') as Tab) || 'rewards';
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab Content */}
      {activeTab === 'rewards' && <StoreRewardsList gymId={gymId} />}
      {activeTab === 'redemptions' && (
