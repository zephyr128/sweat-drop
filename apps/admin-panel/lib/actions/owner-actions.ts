...
async function sendOwnerInvitationEmail(invitation: Record<string, unknown>, gymName?: string) {
  try {
    ...
    const html = buildOwnerInvitationEmailHtml({ gymName, acceptUrl });
    const result = await sendEmail({
      to: invitation.email as string,
      subject: gymName
        ? `You've been invited to manage ${gymName} on SweatDrop`
        : "You've been invited to SweatDrop as a Gym Owner",
      html,
    });

    if (!result.success && result.error !== 'RESEND_API_KEY not configured') {
      console.error('Owner invitation email failed:', result.error);
    }
  } catch (error) {
