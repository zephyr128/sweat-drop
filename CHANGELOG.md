...
### Fixed
- **Mobile app: workout metrics no longer inherit distance and calories from the prior session on the same FTMS machine.** FTMS treadmills, bikes, and cross-trainers emit `total_distance` and `total_energy` as machine-scoped cumulative totals that survive across our BLE connect/disconnect cycle. A fresh session was inheriting the previous user's km and kcal the moment the first FTMS notification arrived, polluting distance/calorie challenges, Kilometer Club / Marathoner badge progress, and leaderboards. Fix introduces a per-session `applyCumulativeBaseline` helper (`apps/mobile-app/lib/ble/cumulativeBaseline.ts`) that anchors on the first non-zero device reading and computes session-relative deltas — the same pattern already used for crank-revolution-based drop calculation. Handles BLE drop/reconnect (refs survive, no re-anchor), machine mid-session reset (carryOver preserves earned credit, re-anchors at new low value), and sanity-ceiling guard for malformed first packets. The `calories_source` predicate at finalize now uses the baseline-anchored sentinel so `'device'` is correctly assigned even when the session produced <1 kcal of adjusted work. Patch points: `apps/mobile-app/app/workout.tsx` (generic FTMS distance branch, generic FTMS calories branch, treadmill display distance branch, treadmill display calories branch). Plan: `docs/plans/bugfix_workout_metrics_baseline_carryover_from_prior_session.md`.

### Added
- **Backend: Multi-gym notification differentiation — all notification senders now identify the originating gym.** In production, multi-gym users had no way to tell which gym sent a push notification (OS banner shows only title + body; iOS forces the app icon). Every gym-scoped sender now (a) suffixes the notification title with the gym name (`"Title — Gym Name"`) and (b) includes `gym_id`, `gym_name`, and `gym_logo_url` in the `data` payload so the in-app inbox can render gym logos and gym name chips. Changes by function:
  - **`streak-reminder`** — Restructured from one global bulk push to a per-gym loop (mirrors `send-happy-hour-reminders` pattern). For each active gym, finds members with at-risk streaks who haven't visited today, sends `"🔥 Streak at risk! — [Gym Name]"`. Users in multiple gyms receive one reminder per gym (visiting any gym saves the global streak). (`backend/supabase/functions/streak-reminder/index.ts`)
  - **`re-engagement`** — Same per-gym restructure. `"💪 We miss you! — [Gym Name]"` (7-day) and `"📣 It's been 2 weeks! — [Gym Name]"` (14-day). Uses `profiles.last_visit_date` (global field) per-gym membership for attribution. (`backend/supabase/functions/re-engagement/index.ts`)
  - **`drops-expiry-warning`** — Adds `gym_id` to `drops_transactions` select; groups by `user_id + gym_id` instead of `user_id` alone. Pre-fetches gym name + logo for all gym groups. Sends per-gym-group push: `"💧 Drops expiring soon — [Gym Name]"` (30d) and `"⚠️ Drops expiring in 7 days — [Gym Name]"` (7d). Users with drops from multiple gyms receive one notification per gym. (`backend/supabase/functions/drops-expiry-warning/index.ts`)
  - **`finalize-arena`** — Adds `gym_id` to `sweat_arenas` query; pre-fetches gym name + logo for all arenas in the run. Winner push title: `"🏆 Arena Prize Won! — [Gym Name]"`; `gym_logo_url` added to `pushData`. Participant (non-winner) push title: `"🏁 Arena Ended — [Gym Name]"`; `gym_id`, `gym_name`, `gym_logo_url` now in data. (`backend/supabase/functions/finalize-arena/index.ts`)
  - **`distribute-leaderboard-prizes`** — Adds `logo_url` to `gyms` query. Push data now includes `gym_name` and `gym_logo_url` alongside existing `gym_id`. Title: `"🏆 You Won a Leaderboard Prize! — [Gym Name]"`. (`backend/supabase/functions/distribute-leaderboard-prizes/index.ts`)
  - **`notify-arena-participants`** — Adds `gym_id` to `sweat_arenas` query; looks up gym name + logo. Winner push: `"🏆 Arena Prize Won! — [Gym Name]"` + gym fields in data. Participant push: `"🏁 Arena Ended — [Gym Name]"` + gym fields in data. (`backend/supabase/functions/notify-arena-participants/index.ts`)
  - **`send-prize-ready-push`** — Adds `logo_url` to existing gym query. Push data now includes `gym_name` and `gym_logo_url`. Title: `"🎁 Your prize is ready! — [Gym Name]"`. (`backend/supabase/functions/send-prize-ready-push/index.ts`)
  - **`process-campaigns`** — Pre-fetches gym name + `logo_url` for all campaign `gym_id`s before the send loop. Title suffixed: `"[Campaign Title] — [Gym Name]"`. Push data includes `gym_name` and `gym_logo_url` (in addition to existing `gym_id` from campaign). Tokenless inbox rows use the same suffixed title. (`backend/supabase/functions/process-campaigns/index.ts`)
  - **`_shared/send-push-request.ts`** — Exports new `GymPushContext` interface and JSDoc block documenting the `gym_id` / `gym_name` / `gym_logo_url` data convention and the `"Title — Gym Name"` suffix pattern for all gym-scoped callers. No runtime change. (`backend/supabase/functions/_shared/send-push-request.ts`)
  - **`send-happy-hour-reminders`** — Already compliant; no changes needed.
  - **Plan:** `docs/plans/feature_multigym_notification_differentiation.md` — Steps 1, 2, 4, 5 (edge-function-agent scope) complete. Steps 3, 6, 7 (mobile-coder scope: inbox gym logo UI, iOS subtitle, i18n) remain.
...
### Fixed
- **Push notification deep-link loop fixed** (`apps/mobile-app/app/_layout.tsx`): Opening the app from a push notification and navigating back to home would re-open the notification screen in an infinite loop (required force-quitting the app). Root cause: `useThrottledRouter()` returns a new object literal on every render; including it in `useCallback([router, session?.user])` meant `handleNotificationTap` was recreated on every render, which re-ran the `addNotificationListeners` effect and re-registered `addNotificationResponseReceivedListener` on every render. On Android, a newly registered listener replays the notification that launched the app, so `router.push(deepLink)` fired on every re-render. Fix: read `session` via a ref (no dependency) and depend only on the individually-memoised `router.push`/`router.replace` methods (stable `useCallback` refs inside `useThrottledRouter`) so `handleNotificationTap` identity never changes. Added a 2-second dedup window to guard against the rare case where both `getInitialNotification` and the response listener deliver the same cold-start notification.
